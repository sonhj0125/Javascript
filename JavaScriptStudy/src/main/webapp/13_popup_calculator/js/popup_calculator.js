window.onload = function () {
        ...
        html += `<tr>
                    ...
                    <td>
                        <ul>
                            ...
                            <li><label>가격 : </label><span id='${item.productId}'>${item.price.toLocaleString('en')}</span></li> <!-- 3자리마다 콤마 찍기 -->
                            <li><label>주문수량 : </label><input type='number' min='0' max='20' value='0' id='${item.productId}'/></li>
                        </ul>
                    </td>
                    <td>`;
                    
                    item.option.forEach(function(elt, idx) {
    ...
    for(let img of img_list) {
        img.onclick = () => {
            // console.log(img.src);
            /*
                http://192.168.0.202:5500/13_popup_calculator/images/monitor.jpg
                http://192.168.0.202:5500/13_popup_calculator/images/cam.jpg
            */
...
/*
    function open_popup(src) {
        // 팝업창 띄우기
        // [예시] window.open("popup_contents.html", "my_popup", "left=100px, top=100px, width=400px, height=350px");
        ...
        // popup.document.writeln(`<img src='${src}' />`); 와 같음
...
*/
    ...
    const open_popup = src => {
        ...
        popup.document.writeln("</body>");
        popup.document.writeln("</html>");
    }; // end of const open_popup = function(src) {} --------
    ...
    for(let input_number of input_number_list) {
        ...
        input_number.addEventListener('change', () => {
            ...
            const str_price = document.querySelector("table#tbl span#" + input_number.id).innerText.split(",").join("");  // String 타입
