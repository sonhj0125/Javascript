window.onload = function() {
    ...
    const image_width = 900;    // 한번 이동 시 이미지 크기만큼 이동한다.
    ...
    const func_next = function() {
        if(current_index < 3) { // 현재 인덱스 번호가 마지막(지금은 3)이 아닌 경우
            ...
            images.style.transform = `translateX(${positionValue}px)`;   // x축 방향(가로방향)으로 positionValue 만큼 이동하도록 변형시킨다.
            document.querySelector("h2#msg").innerHTML = "";

            current_index++;     // 현재 인덱스 번호를 1 증가시킨다.
        }
        else {  // 현재 인덱스 번호가 마지막인 경우
            btn_next.setAttribute('disabled', ' true');     // 마지막 사진일 때 다음버튼을 비활성화로 만든다.

            // 또는
            // btn_next.disabled = true;     // 마지막 사진일 때 다음버튼을 비활성화로 만든다.
    ...
    const func_previous = function() {
        if(current_index > 0) { // 현재 인덱스 번호가 처음이 아닌 두번째 이상인 경우
            ...
            // 또는
            // btn_next.disabled = ture;   // 다음 버튼을 비활성화 상태로 만든다.
            ...
            btn_previous.setAttribute('disabled', ' true');     // 처음 사진일 때 이전버튼을 비활성화로 만든다.

            // 또는
            // btn_previous.disabled = true;     // 처음 사진일 때 이전버튼을 비활성화로 만든다.
